...
def inference_pose(save_path):
    if os.path.exists(save_path):
        return

    # 加载您的模型
    pose_agent = YourModelClass(cfg)
    pose_agent.load_ckpt(
        model_dir=cfg.pretrained_model_path,
        model_path=True,
    ...
    for i, test_batch in enumerate(tqdm(dataloader, desc="Pose Inference")):
        ...
        with torch.no_grad():
            outputs = pose_agent(
                rgb=batch_sample['rgb'],
                mask=batch_sample['mask'],
                partial_pcl=batch_sample['partial_pcl']
            )
