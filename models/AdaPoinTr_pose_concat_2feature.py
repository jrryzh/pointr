...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.increase_dim_for_pose = nn.Sequential(
            nn.Linear(encoder_config.embed_dim, 1024),
            nn.GELU(),
            nn.Linear(1024, global_feature_dim)
        )
    ...
    def forward(self, xyz):
        ...
        pose_feature = self.increase_dim_for_pose(x) # B 1024 N # 和global_feature维度一样
        pose_feature = torch.max(pose_feature, dim=1)[0] # B 1024 # max后变为1024维
        ...
        if self.training:
            ...
            if self.pre_global_feature:
                return q, coarse, denoise_length, global_feature, pose_feature
            else:
            ...
            if self.pre_global_feature:
                return q, coarse, 0, global_feature, pose_feature
            else:
                return q, coarse, 0
...
class AdaPoinTr_Pose_concat_2feature(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
    ...
    def forward(self, xyz):
        q, coarse_point_cloud, denoise_length, pre_global_feature, pose_feature = self.base_model(xyz) # B M C and B M 3
        ...
        ############# 添加分支预测 rotate trans size ############
        concated_global_feature = torch.cat([pose_feature, pre_global_feature, global_feature], dim=1)
