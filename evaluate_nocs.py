...
def estimate():
    ...
    builder.load_model(base_model, opt.ckpts, logger = logger)
    base_model.to(opt.local_rank)
    base_model.eval()
    print('Load_model in {}.'.format(opt.ckpts))
    ...
    for path in tqdm(img_list):
        ...
        for i in range(num_insts):
            ...
            if opt.pcd_isSave:
                ...
                image_id = path.split('/')[-1]
                create_folder(save_folder)
                save_to_obj_pts(_backproj_pcd, os.path.join(save_folder, '{}_{}_{}_original_cam.obj'.format(scene_id, image_id, categories[cate_id + 1])))
                # 其他保存
                # save_to_obj_pts(obsv_pcd, os.path.join(save_folder, '{}_{}_{}_pre_input.obj'.format(scene_id, image_id, categories[cate_id + 1]))) # 观察下方向
                save_to_obj_pts(input_pcd, os.path.join(save_folder, '{}_{}_{}_final_input.obj'.format(scene_id, image_id, categories[cate_id + 1])))
                save_to_obj_pts(coarse_points_np, os.path.join(save_folder, '{}_{}_{}_coarse_complete.obj'.format(scene_id, image_id, categories[cate_id + 1])))
                # save_to_obj_pts(denset_poins_np, os.path.join(save_folder, '{}_{}_{}_dense_complete.obj'.format(scene_id, image_id, categories[cate_id + 1])))
            '''
            ###############################
            ...
            #     save_to_obj_pts(transformed_bbox_3d.transpose(), os.path.join(save_folder, '{}_{}_{}_predicted_bbox.obj'.format(scene_id, image_id, categories[cate_id + 1])))
        ...
        save_path = os.path.join(save_folder, 'results_{}.pkl'.format(image_short_path))
        with open(save_path, 'wb') as f:
            cPickle.dump(result, f)
        # TODO: DEBUG: draw result
        # _draw_detections(raw_rgb[:, :, ::-1], save_folder, 'd435', f'{img_count:04}', intrinsics, f_sRT, f_size, f_class_id,
        #     [], [], [], [], [], [], draw_gt=False, draw_nocs=False)
        if opt.img_isSave:
            scene_id = path.split('/')[-2].split('_')[-1]
            image_id = path.split('/')[-1]
            gt_img = raw_rgb[:, :, ::-1].copy()
            draw_detections(raw_rgb[:, :, ::-1], save_folder, 'pred', f'{scene_id}_{image_id}', intrinsics, f_sRT, f_size, f_class_id)  
            # draw_detections(gt_img, save_folder, 'gt', f'{img_count-1:04}', intrinsics, gts['poses'], gts['size'], gts['class_ids'])
    # write statistics
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpts', default='ckpt-last.pth',  help='ckpt')
    parser.add_argument('--resume', type = bool, default = True, help = 'autoresume training (interrupted by accident)')
    parser.add_argument('--local-rank', type=int, default=0) # 更新的pytorch版本用 local-rank 而非 local_rank
    ...
    log_file = os.path.join(opt.experiment_path, f'{timestamp}.log')
    logger = get_root_logger(log_file=log_file, name='nocs_eval')
    model_config = get_config(opt, logger = logger)
    ...
    # 添加save folder
    save_folder = os.path.join(result_folder, opt.exp_name)
