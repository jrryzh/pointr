...
from utils.omni6d.data_augmentation import defor_2D, get_rotation
from utils.omni6d.data_augmentation import data_augment
from utils.omni6d.datasets_utils import aug_bbox_DZI, get_2d_coord_np, crop_resize_by_warp_affine
from utils.omni6d.sgpa_utils import load_depth, get_bbox
from utils.omni6d.config import get_config
from utils.omni6d.misc import get_rot_matrix, get_pose_representation
from utils.omni6d.tracking_utils import add_noise_to_R
from utils.omni6d.transforms import *
...
class Omni6DPoseDataSet(data.Dataset):
    def __init__(self, 
        ...
        self.obj_meta = ObjectMetaData.load_json(
            "utils/omni6d/obj_meta.json" if mode != 'real'
                else "utils/omni6d/real_obj_meta.json")
        ...
        if cfg.load_per_object:
            ...
            for img_path in self.img_list:
                # from data/Omni6DPose/ROPE/000000/color/000000_ to data/Omni6DPose/ROPE/000000/meta/000000_meta.json
                gts: ImageMetaData = Dataset.load_meta((img_path + "meta.json").replace('color', 'meta')) # 似乎之前路径都有问题 gts: ImageMetaData = Dataset.load_meta(img_path + "meta.json")
    ...
    def __getitem__(self, index):
        ...
        depth = Dataset.load_depth(img_path.replace('color', 'depth') + ('depth_syn' if self.cfg.perfect_depth else 'depth') + '.exr') # 依旧路径有问题 depth = Dataset.load_depth(img_path + ('depth_syn' if self.cfg.perfect_depth else 'depth') + '.exr')
        depth[depth > 1e3] = 0
        mask = Dataset.load_mask(img_path.replace('color','mask') + 'mask.exr')
