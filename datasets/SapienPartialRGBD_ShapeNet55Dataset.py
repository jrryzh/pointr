...
class SapienPartialRGBD_ShapeNet(data.Dataset):
    def __init__(self, config):
        ...
        if self.subset == 'train':
            self.data_list_file = os.path.join(self.data_root, f'500view_rgb_shapenet_train_list.txt')
        elif self.subset == 'val' or self.subset == 'test':
            self.data_list_file = os.path.join(self.data_root, f'500view_rgb_shapenet_test_list.txt')
            ...
            if self.subset == 'train':
                for idx in range(500):
                    self.file_list.append({
                            ...
                            'mask_path': os.path.join(line, f'{idx:04}_mask.png')
                        })
